...
# Link: https://github.com/akihirao/how2cook/blob/main/vcf2newhybs.py
...
# Hank Note: 
#   I had to download legacy version of pyvcf in my environment to run this.
...
# Edited by Hank ---------------
#   The previous version didn't include a '1/0' scenario
def convert_gt(gt):
    if gt is None:
        return 0
    ...
    if gt == "0/0":
        ...
        # catch unexpected cases (e.g., multi-allelic like 0/2)
        return 0
...
def vcf2newhybs(inp, out):
    # Removed reliance on 'no_selected_loci' --> include all
    ...
    with open(out, 'w') as outfh:
        ...
        selected_loci_id = sorted(random.sample(list(range(0,no_loci)),no_loci))
        ...
        outfh.write('NumLoci' + '\t' + str(no_loci) + '\n')
