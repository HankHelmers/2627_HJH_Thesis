# INSTALLED FROM https://github.com/salanova-elliott/recom-sim/tree/master
# on May 6th, 2026
...
def read_genepop(genepop_file):
	...
	for line in genepop_file:
		...
		#Sample capture
			...
			alleles = sample_search.group(2) or sample_search.group(3)
			if current_pop == 1:    
				names_pop1.append(sample_search.group(1).rstrip())
				allele_list_pop1.append(alleles.lstrip().split(" "))
			if current_pop == 2:
				names_pop2.append(sample_search.group(1).rstrip())
				allele_list_pop2.append(alleles.lstrip().split(" "))
