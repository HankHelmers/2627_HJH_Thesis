...
def get_commandline_arguments ():
	...
	parser.add_argument("--z_option", required=False, dest="z_option", type=extant_file, help="name and path of the z_option file: 1st column barcode/sample name separated by tab from second column indicating the z_option: usually z0 or z1 or *blank*; if a sample missing fom this file it will be absent from newhybrids dataset", metavar="FILE")
	parser.add_argument("--vcf", required=True, dest="vcf", type=extant_file, help="vcf genotypes", metavar="FILE")
 
    # I added
	parser.add_argument("--outfile", required=True, dest="outfile", help="outfile location")
...
# Removed the z_options
def write_newhybrids_format ( genotype_dict, output_file ):

	line1 = "NumIndivs" + "\t" + str(len(genotype_dict.keys()))
	line2 = "NumLoci" + "\t" + str(len(list(genotype_dict.values())[0]))
	line3 = "Digits	1"
	line4 = "Format	Lumped"
	line5 = "LocusNames" + "\t" + "\t".join( ["locus_"+str(x) for x in range(0,len(list(genotype_dict.values())[0])) ] )
	...
	for idx, sample in enumerate(genotype_dict.keys()):
        ...
        # ADDED "n" option for the sample name
		outl = str(idx+1) + "\tn\t" + sample + "\t" + "\t".join( genotype_dict[sample] )
